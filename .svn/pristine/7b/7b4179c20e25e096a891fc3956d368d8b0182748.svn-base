 ...
 */
	...
	function getEffectifDebutAnnee(listCollaborateurs)
	{
		...
		for(var i=0; i<listCollaborateurs.length; i++){
			if(getCurrentYear() < getCurrentYearCollaborateur(listCollaborateurs[i].date_sortie) && getCurrentYear() > getCurrentYearCollaborateur(listCollaborateurs[i].date_entree)
					|| (getCurrentYear() == getCurrentYearCollaborateur(listCollaborateurs[i].date_sortie) &&	1 < getCurrentMonthCollaborateur(listCollaborateurs[i].date_sortie))){// &&	1 == getCurrentMonthCollaborateur(collaborateurs[i].Datent)
	...
	function getEntreesSortiesCollaborateurs(mois,annee,listCollaborateurs)
	{
		...
		for(var i=0; i<listCollaborateurs.length; i++){
			...
			if(annee ==  getCurrentYearCollaborateur(listCollaborateurs[i].date_entree) && mois == getCurrentMonthCollaborateur(listCollaborateurs[i].date_entree))
				{
				nbCollaborateursEntrees++;
				}
			if(annee ==  getCurrentYearCollaborateur(listCollaborateurs[i].date_sortie) && mois == getCurrentMonthCollaborateur(listCollaborateurs[i].date_sortie))
				{
	...
	function getCollaborateurEntreeByES(moi,listCollaborateurs)
	{
		...
		for(var i=0; i<listCollaborateurs.length; i++){
			...
			if(getCurrentYear() ==  getCurrentYearCollaborateur(listCollaborateurs[i].date_entree) && mois == getCurrentMonthCollaborateur(listCollaborateurs[i].date_entree))
				{
	...
	{ 	//+1 parceque le premier row du chart c'est 0 
		...
		for(var i=0; i<listCollaborateurs.length; i++){
			...
			if(getCurrentYear() ==  getCurrentYearCollaborateur(listCollaborateurs[i].date_sortie) && mois == getCurrentMonthCollaborateur(listCollaborateurs[i].date_sortie))
				{
